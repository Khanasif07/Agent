...
class OneToOneChatVC: BaseVC {
    ...
    @IBOutlet weak var blockInfoLabel: UILabel!
    ...
    override func viewDidLayoutSubviews() {
        ...
        topView.addShadow(cornerRadius: 0, color:AppColors.fontTertiaryColor, offset: CGSize(width: 0, height: 4), opacity: 1, shadowRadius: 8)
        infoView.round(radius: 5)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if !self.infoView.frame.contains(touchLocation) {
    ...
    @IBAction func moreInfoButtonTapped(_ sender: UIButton) {
        UIView.animate(withDuration: 0.2) {
            ...
            self.backgroundView.alpha = 0.50
        }
    }
...
//        AppRouter.goToMyGarageVC(self, userId: inboxModel.userId,userName: inboxModel.firstName, comeFromOtherProfile: true)
    ...
    private func popToInboxView() {
        ...
        for vc in nvc.viewControllers {
            if vc is GarageTabBarController {
                self.navigationController?.popToViewControllerOfType(classForCoder: vc)
            }
        }
        self.pop()
    }
    ...
    private func updateBlockUI() {
        attachmentButton.isHidden = isBlockedByMe || amIBlocked
        textContainerInnerView.isHidden = isBlockedByMe || amIBlocked
        blockInfoLabel.isHidden = !isBlockedByMe && !amIBlocked
...
//                self.createBatch()
        ...
        sendButton.setImage(#imageLiteral(resourceName: "group3669"), for: .normal)
        resetFrames()
    }
...
extension OneToOneChatVC: UITextViewDelegate{
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if text.byRemovingLeadingTrailingWhiteSpaces.isEmpty {
            sendButton.setImage(#imageLiteral(resourceName: "icSend"), for: .normal)
            if text.isEmpty {
                self.resetFrames()
                return
            }
        } else {
            sendButton.setImage(#imageLiteral(resourceName: "icSendActive"), for: .normal)
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case AppUserDefaults.value(forKey: .uid).stringValue:
            ...
            default:
                ...
                senderCell.senderImageView.sd_setImage(with: URL(string: userImage), placeholderImage: #imageLiteral(resourceName: StringConstants.eventImagePlaceHolder) , completed: nil)
                senderCell.senderImageView.addGestureRecognizer(imgTap)
                return senderCell
            }
        default:
            switch model.messageType {
            ...
            default:
                ...
                receiverCell.msgContainerView.backgroundColor = selectedIndexPaths.contains(indexPath) ? AppColors.blackColor : AppColors.blueChatBubble
                receiverCell.msgLabel.textColor = selectedIndexPaths.contains(indexPath) ? AppColors.whiteColor : UIColor.black
                return receiverCell
            }
        }
    }
...
extension OneToOneChatVC{
    ...
    private func startAdminBlockListener() {
        let listener = db.collection(ApiKey.users).document(currentUserId).addSnapshotListener { (document, error) in
            if let document = document?.data() {
                if document[ApiKey.status] as? String == "block" {
    ...
    /// Mark:- Block the send button
    private func blockSendButton(){
        FirestoreController.isReceiverBlocked(senderId: currentUserId, receiverId: inboxModel.userId) { (bool) in
            self.isBlockedByMe = bool
        }
    }
    ...
    private func getLastMessageBeforeBlock(){
        ...
        let uid = AppUserDefaults.value(forKey: .uid).string ?? StringConstants.defaultValue
    ...
    private func createMediaMessage(url: String, imageURL: String = "", type: String) {
        FirestoreController.createMessageNode(roomId: self.roomId, messageText: "", messageTime: FieldValue.serverTimestamp(), messageId: self.getMessageId(), messageType: type, messageStatus: 1, senderId: self.currentUserId, receiverId: self.inboxModel.userId, mediaUrl: url, blocked: self.isBlockedByMe, thumbNailURL: imageURL)
        let attachmentText = type == MessageType.image.rawValue ? "Photo Attachment" : "Video Attachment"
        FirestoreController.createLastMessageNode(roomId: self.roomId, messageText: attachmentText, messageTime: FieldValue.serverTimestamp(), messageId: self.getMessageId(), messageType: type, messageStatus: 1, senderId: self.currentUserId, receiverId: self.inboxModel.userId, mediaUrl: url, blocked: self.isBlockedByMe, thumbNailURL: imageURL)
    }
    ...
    /// Mark:- Check if the user is blocked before sending the message
    private func sendMessageBlockCheck(){
        if isBlockedByMe{
            updateIfSenderIsBlocked()
        } else {
            //CommonFunctions.showAlert(msg: StringConstants.UnblockUserMessage)
        }
    }
    ...
    private func createRoom() {
        ...
        let _ : [String: Any] = [ApiKey.roomId : roomId,
                                          ...
                                          ApiKey.roomName:"",
                                          ApiKey.roomType:StringConstants.single,
                                          ApiKey.userInfo: userInfoDict,
                                          ApiKey.userTypingStatus: userTypingStatus]
        
        FirestoreController.createRoomNode(roomId: roomId, roomImage: roomImageURL, roomName: "", roomType: StringConstants.single, userInfo: userInfoDict, userTypingStatus: userTypingStatus)
...
//                                self.db.collection(ApiKey.inbox).document(self.currentUserId).collection(ApiKey.chat).document(self.inboxModel.userId).updateData([ApiKey.unreadMessages: 0])
                        ...
                        if (message.messageTime.dateValue().convertToString() == self.tempTime.dateValue().convertToString()) || self.messageListing.isEmpty {
